...
class _SiparisGrafigiState extends State<SiparisGrafigi> {
  ...
  DateTime _baslangicFromFiltre() {
    ...
    switch (secilenFiltre) {
      case 'Günlük':   return now.subtract(const Duration(days: 6));
      case 'Haftalık': return now.subtract(const Duration(days: 30));
      case 'Aylık':    return now.subtract(const Duration(days: 90));
      case 'Yıllık':   return now.subtract(const Duration(days: 365));
      default:         return now.subtract(const Duration(days: 6));
    }
  }
                ...
                child: StreamBuilder<Map<DateTime, int>>(
                  ...
                  builder: (context, snap) {
                    ...
                    final spots = List.generate(
                      sayilar.length, (i) => FlSpot(i.toDouble(), sayilar[i].toDouble()),
                    );

                    final maxY = sayilar.isEmpty ? 0.0 : sayilar.reduce(math.max).toDouble();
                    final yInterval = _niceInterval(maxY);
                    final xStep = sortedDays.length <= 12 ? 1 : (sortedDays.length / 12).ceil();

                    return LineChart(
                      LineChartData(
                        minX: 0,
                        maxX: (sayilar.isEmpty ? 0 : sayilar.length - 1).toDouble(),
                        minY: 0,
                          ...
                          bottomTitles: AxisTitles(
                            sideTitles: SideTitles(
                              ...
                              getTitlesWidget: (value, meta) {
                                ...
                                if (index % xStep != 0) return const SizedBox.shrink();

                                final d = sortedDays[index];
                                final label = DateFormat('M/d').format(d);
                                return Transform.rotate(
                                  angle: -0.785398163, // -45°
                                  child: Text(label, style: const TextStyle(color: Colors.black87, fontSize: 10)),
                                );
                              },
                            ),
                          ),
                          ...
                          LineChartBarData(
                            ...
                            belowBarData: BarAreaData(
                              ...
                              gradient: LinearGradient(
                                colors: [Colors.green.withOpacity(0.15), Colors.green.withOpacity(0.0)],
                                stops: const [0.5, 1.0],
                          ...
                          getTouchedSpotIndicator: (barData, spotIndexes) {
                            return spotIndexes.map((_) {
                              return const TouchedSpotIndicatorData(
                                FlLine(color: Colors.blueGrey, strokeWidth: 1.2, dashArray: [8, 3]),
                                FlDotData(show: true),
                              );
                            }).toList();
                          },
                          touchTooltipData: LineTouchTooltipData(
                            getTooltipItems: (touched) {
                              return touched.map((barSpot) {
                                final i = barSpot.x.toInt().clamp(0, sortedDays.length - 1);
