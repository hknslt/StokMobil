              ...
              background: Container(
                decoration: BoxDecoration(
                  ...
                  gradient: LinearGradient(
                    ...
                    colors: [Renkler.anaMavi, Renkler.kahveTon.withOpacity(.85)],
                  ),
                ),
              ),
            ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: _ProfileHeader(
                name: displayName?.isNotEmpty == true ? displayName! : 'Kullanıcı',
                email: email,
                onEdit: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const KullaniciBilgileriSayfasi()),
                  );
                },
              ),
            ),
              ...
              children: [
                _SettingTile(
                  ...
                  onTap: () => Navigator.push(
                    context, MaterialPageRoute(builder: (_) => const KullaniciBilgileriSayfasi())),
                ),
                _SettingTile(
                  ...
                  onTap: () => Navigator.push(
                    context, MaterialPageRoute(builder: (_) => const SifreSifirlamaSayfasi())),
                ),
              ],
            ),
          ),
            ...
            child: _Section(
              ...
              children: [
                _SettingTile(
                  ...
                  onTap: () => Navigator.push(
                    context, MaterialPageRoute(builder: (_) => const BildirimAyarlarSayfasi())),
                ),
              ],
            ),
          ),
...
class _ProfileHeader extends StatelessWidget {
  ...
  const _ProfileHeader({required this.name, required this.email, required this.onEdit});
...
class _Avatar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final initial = name.trim().isNotEmpty ? name.trim().split(RegExp(r'\s+')).map((e) => e[0]).take(2).join() : 'K';
    final cs = Theme.of(context).colorScheme;
    return Container(
      width: 56, height: 56,
      decoration: BoxDecoration(
      ...
      children: items.map((it) {
        return Expanded(
          child: Padding(
            ...
            child: _QuickCard(
              icon: it.icon, label: it.label, onTap: it.onTap, color: cs.primary,
            ),
          ),
        );
      }).toList()..removeLast(),
...
class _SettingTile extends StatelessWidget {
  ...
  final VoidCallback? onTap;
  const _SettingTile({required this.leading, required this.title, this.subtitle, this.onTap});
...
class _StaticInfoTile extends StatelessWidget {
  ...
  final String title;
  final String value;
  const _StaticInfoTile({required this.leading, required this.title, required this.value});
